...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user || !existingPlan) {
      return NextResponse.json(
        { success: false, message: "User or active plan not found" },
        { status: 404 }
      );
    }
    ...
    if (!newPlanDetails) {
      return NextResponse.json(
        { success: false, message: "Invalid new plan selected" },
        { status: 400 }
      );
    }

    const existingTransaction = await Plan.findOne({
    ...
    newExpiryDate.setMonth(
      newExpiryDate.getMonth() + PLAN_DURATIONS_MONTHS[newPlanDetails.title]
    );

    user.plan = newPlanDetails.title;
    user.listings +=
      (newPlanDetails.listings ?? 0) - (existingPlan.listings ?? 0);
    user.premiumBadging +=
      (newPlanDetails.premiumBadging ?? 0) - (existingPlan.premiumBadging ?? 0);
    ...
    const savedPlan = await existingPlan.save(); // 📧 Send invoice email after successful upgrade
    try {
      // The user object is already available from the database query
      if (user) {
