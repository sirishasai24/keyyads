...
import { sendInvoiceEmail } from "@/helpers/sendInvoiceEmail"; // Import the invoice mailer function
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      ...
      !razorpay_signature ||
      !plan ||
      !currentUserPlanDetails
    ) {
      return NextResponse.json(
        { success: false, message: "Missing payment or plan details" },
        { status: 400 }
      );
    }
    ...
    const existingPlanDetails = await Plan.findOne({
      userId: user._id,
      planTitle: currentUserPlanDetails.planTitle,
    });

    if (!existingPlanDetails) {
      console.error(
        `Attempted to renew plan for user ${user._id} but no matching existing plan details found for title: ${currentUserPlanDetails.planTitle}`
      );
      return NextResponse.json(
        { success: false, message: "No active plan found to renew." },
        { status: 404 }
      );
    }
    ...
    if (typeof planDurationMonths === "undefined") {
      return NextResponse.json(
        { success: false, message: "Invalid plan title provided for renewal." },
        { status: 400 }
      );
    }
    ...
    if (currentExpiryDate < now) {
      ...
      console.log(
        `Current plan for user ${user._id} expired. Renewing from current date.`
      );
    }
    ...
    const savedPlan = await existingPlanDetails.save();

    user.planId = savedPlan._id; // Update planId to refer to the renewed plan document
    await user.save();

    console.log(
      `Plan '${plan.title}' renewed for user ${
        user._id
      }. New expiry: ${newExpiryDate.toISOString()}`
    ); // 📧 Send invoice email after successful renewal

    try {
      // The user and plan objects are already available from the database query
      if (user) {
        await sendInvoiceEmail({
          toEmail: user.email,
          planDetails: savedPlan.planDetailsSnapshot,
          transactionId: savedPlan.transactionId,
          startDate: new Date(savedPlan.createdAt), // Use createdAt as the new start date for the renewed plan
          expiryDate: new Date(savedPlan.expiryDate),
          username: user.username,
        });
      }
    } catch (emailError) {
      console.error("Failed to send invoice email:", emailError); // The main API response will not fail due to a mailer error
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Internal server error during plan renewal",
        error: error,
      },
      { status: 500 }
    );
